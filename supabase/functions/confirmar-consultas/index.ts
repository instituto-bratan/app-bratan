...
import { db, json, lerIntegracao, registrarEvento, respostaDesligada, telefoneE164 } from "../_shared/integracoes.ts";
...
Deno.serve(async (request) => {
  ...
  async function toque(item: { id: string; inicio: string; paciente: string | null; telefone: string | null; profissional: string | null }, etapa: "48h" | "2h") {
    const telefone = telefoneE164(item.telefone ?? "");
  ...
  await registrarEvento(client, { chave: "whatsapp", direcao: "SAIDA", status: "OK", resumo: `Confirmação em dois toques: ${enviados} enviado(s) (${(a48 ?? []).length} de 48 h, ${(a2 ?? []).length} de 2 h)${semTelefone ? ` · ${semTelefone} sem telefone` : ""}` });
