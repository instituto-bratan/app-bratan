create table if not exists public.checkin_event_codes (
  ...
  constraint checkin_event_codes_label_len check (char_length(trim(label)) between 3 and 80),
  constraint checkin_event_codes_preview_len check (char_length(code_preview) between 2 and 12)
);

create index if not exists idx_checkin_event_codes_lookup
on public.checkin_event_codes(checkin_type, event_date, active);

create unique index if not exists uq_checkin_event_codes_active_hash
on public.checkin_event_codes(checkin_type, event_date, code_hash)
where active = true;

drop trigger if exists trg_checkin_event_codes_updated_at on public.checkin_event_codes;
create trigger trg_checkin_event_codes_updated_at
before update on public.checkin_event_codes
for each row
execute function public.set_updated_at();

create or replace function public.normalized_checkin_code_hash(_validation_code text)
returns text
language sql
immutable
security definer
...
as $$
  select cec.id
  from public.checkin_event_codes cec
  where cec.checkin_type = _checkin_type
    and cec.event_date = _event_date
    and cec.active = true
    and cec.code_hash = public.normalized_checkin_code_hash(_validation_code)
    and (cec.expires_at is null or cec.expires_at > now())
...
alter table public.checkin_event_codes enable row level security;

drop policy if exists "checkin_event_codes_select_admin" on public.checkin_event_codes;
create policy "checkin_event_codes_select_admin"
on public.checkin_event_codes
for select
to authenticated
using (public.is_coordenacao(auth.uid()));

drop policy if exists "checkin_event_codes_write_admin" on public.checkin_event_codes;
create policy "checkin_event_codes_write_admin"
on public.checkin_event_codes
for all
to authenticated
using (public.is_coordenacao(auth.uid()))
...
grant execute on function public.normalized_checkin_code_hash(text) to authenticated;
grant execute on function public.valid_checkin_event_code(public.checkin_type, text, date) to authenticated;
