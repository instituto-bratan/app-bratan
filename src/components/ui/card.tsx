...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("ios-glass-quiet rounded-lg border bg-card text-card-foreground shadow-calm", className)}
      {...props}
    />
  ),
);
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3 ref={ref} className={cn("text-lg leading-tight text-brand-musgo sm:text-xl", className)} {...props} />
  ),
);
