...
test("régua do Instituto: impostos % do líquido, lucro cota fixa, executor 50% do lucro bruto do produto (coluna S), operacional é o resto", () => {
  ...
  const env = li.repartir(10000, 3476.74, r, 1904.76);
  assert.equal(env.impostos, 1660, "16,6% dos 10 mil líquidos");
  assert.equal(env.medicoExecutor, 1738.37, "metade do médico: 50% do lucro bruto do Plano (3.476,74) = 1.738,37");
  assert.equal(env.lucro, 1904.76, "a cota do dia, não um percentual");
  assert.equal(env.operacional, 4696.87, "o que sobra para a clínica gastar");
...
test("planilha de precificação no motor: o item da comanda vira a coluna P (lucro bruto) e a S é 50% dela", () => {
  ...
  assert.equal(li.lucroBrutoDoItem(item("TRATAMENTO", 6997, "Programa de acompanhamento 6 meses")), 3476.74, "Plano (planilha OFICIAL de 15/09): 6.997 − 1.819,22 imposto 26% − 699,70 comissão 10% − 660 nutri − 341,34 sala");
  ...
  assert.equal(li.lucroBrutoDoItem(item("CONSULTA", 1500, "Consulta Black")), 846.22);
  assert.equal(li.lucroBrutoDoItem(item("CONSULTA", 1100, "Consulta Diamond")), 590.22, "Diamond (OFICIAL): 1.100 − 286 NF 26% − 110 comissão (10%) − 113,78 sala");
  assert.equal(li.lucroBrutoDoItem(item("TRATAMENTO", 3900, "Teste genético")), 2458.07, "teste genético ainda sem o custo do kit (OFICIAL, comissão 10%)");
  ...
  assert.equal(li.lucroBrutoDoItem(item("TRATAMENTO", 590, "Tirzepatida 40 un")), 279.05, "tirzepatida até 49 un continua com comissão de 1%");
  assert.equal(li.lucroBrutoDoItem(item("TRATAMENTO", 1990, "")), 1053.58, "1.990 só pode ser Ferinject: preço decide");
  assert.equal(li.lucroBrutoDoItem(item("TRATAMENTO", 1000, "Testosterona blend 3ml")), 557.66, "valor fora da tabela = planilha com F = 1.000 e B = 2: 1.000 × (1 − 11,48% − 10%) − 2 × (106,66 consumível + 7,11 poltrona)");
  assert.equal(li.lucroBrutoDoItem(item("TRATAMENTO", 800, "Soro especial")), 397.51, "produto desconhecido: fração do Plano (49,7%)");
  assert.equal(li.lucroBrutoDoItem(item("CONSULTA", 2500, "")), 1486.22, "consulta avulsa Pix");
  assert.equal(li.lucroBrutoDoItem(item("SINAL", 500, "")), 291.56);
  assert.equal(li.lucroBrutoDoItem(item("BIOIMPEDANCIA", 200, "")), 138.08, "mapeamento corporal");
  ...
  assert.equal(li.lucroBrutoNaComanda(comanda), 4962.96, "coluna P somada (1.486,22 + 3.476,74)");
  assert.equal(li.repartir(0, li.lucroBrutoNaComanda(comanda), { impostos: 0, lucroMensal: 0, medicoExecutor: 50 }, 0).medicoExecutor, 2481.48, "metade do médico: 50% de 4.962,96");
});
...
test("explicação item a item (10/09): a parte do Dr. Daniel abre comanda por comanda e bate com a planilha", () => {
  ...
  assert.equal(plano.lucroBrutoTabela, 3476.74);
  assert.equal(plano.quantidade, 1, "4.846 num Plano de 6.997 = 1 Plano");
  assert.equal(plano.lucroBruto, 2100.1, "planilha com F = 4.846: 4.846 × (1 − 26% − 10%) − 1.001,34 fixos (660 nutri + 341,34 sala)");
  assert.equal(plano.custoFixo, 1001.34, "fixos do Plano = 341,34 de sala (por unidade) + 660 de repasse nutri (da linha)");
...
test("catálogo compartilhado: o Fechamento grava o nome oficial e a comanda fecha exatamente no valor recebido", () => {
  ...
  assert.equal(li.lucroBrutoDoItem(cheio[0]), 3476.74, "o item gravado pelo Fechamento cai na coluna P do Plano");
  assert.equal(li.lucroBrutoDoItem(cheio[2]), 871.31, "2 vitaminas D pelo preço cheio = 2 × 435,66, igual à planilha com B = 2");
...
test("planilha do dia: taxas saem antes; executor é a coluna S; lucro é a cota; crédito conta no lançamento e fica disponível em D+1", () => {
  ...
  assert.equal(dia1.lucroBrutoProdutos, 5261.99, "coluna P: consulta 3.003 × 59,4% + Plano de 6.997 pela linha da planilha");
  ...
  assert.equal(dia1.reservado.medicoExecutor, 2631, "a metade do médico: 50% do lucro bruto dos produtos");
  assert.equal(dia1.reservado.lucro, 1000, "a cota do dia útil");
  assert.equal(dia1.reservado.operacional, 5255.6, "9.874 − 987,40 − 2.631 − 1.000");
  ...
  assert.equal(dia2.reservado.medicoExecutor, 291.56, "sinal de 1.000 também tem a metade do médico (58,3% × 50%)");
  assert.equal(dia2.reservado.lucro, 1000, "a cota vale mesmo num dia fraco");
  assert.equal(dia2.reservado.operacional, -391.56, "1.000 − 100 de imposto − 291,56 do médico − 1.000 de cota");
  assert.equal(dia2.acumulado.reservado.operacional, 4864.04, "9.874 − 987,40 de imposto − 2.631 do médico − 1.391,56 de cota");
  assert.equal(p.totais.total, 11000);
  assert.equal(p.totais.liquido, 10874);
  assert.equal(p.totais.prescrito, 11000);
  assert.equal(p.totais.lucroBrutoProdutos, 5845.11);
...
test("planilha do dia: contas pagas caem no envelope certo; obra, provisão e tarifa da maquininha ficam fora", () => {
  ...
  assert.equal(dia.reservado.medicoExecutor, 2972.44, "consulta de 10.000: lucro bruto 59,4% → metade do médico");
  assert.equal(dia.reservado.operacional, 5027.56, "10.000 − 1.000 de imposto − 2.972,44 do médico − 1.000 de cota");
  assert.equal(dia.acumulado.saldo.operacional, -272.44, "5.027,56 reservados − 5.300 gastos (agora com CEO e médico fixo)");
...
test("avaliação instantânea (Passo 1): onde a clínica está, sem maquiar, com o prescrito do mês", () => {
  ...
  perto(mes.lucroBrutoProdutos, 52616.95, 0.02, "coluna P do mês: 30 mil × 59,4% + 70 mil × 49,7% → pela régua o executor levaria ~26,3 mil");
...
test("repasses: provisionado é o acumulado da régua até hoje; transferido são as contas pagas na categoria; falta é a diferença — e dívida do envelope do lucro conta", () => {
  ...
  assert.equal(r.medicoExecutor.provisionado, 3476.74, "2 × 1.738,37 (a metade do médico no Plano)");
  assert.equal(r.medicoExecutor.transferido, 2000);
  assert.equal(r.medicoExecutor.falta, 1476.74);
  ...
  assert.match(li.fraseDoRepasse(r.medicoExecutor, (v) => `R$${v}`), /Provisionado até 02\/09: R\$3476\.74 · já transferido para o Dr\. Daniel: R\$2000 → falta transferir R\$1476\.74/);
